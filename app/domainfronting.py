...
class domainfronting_handler(socketserver.BaseRequestHandler):
    ...
    def request_in_whitelist(self, socket_client_request_host, socket_client_request_port):
        if len(self.server.whitelist_request) == 0:
            return True

        for whitelist_request in self.server.whitelist_request:
    ...
    def handle(self):
        try:
            ...
            self.server.close_request(socket_client)
            return
        ...
        try:
            ...
            self.server.close_request(socket_client)
            self.server.close_request(socket_server)
